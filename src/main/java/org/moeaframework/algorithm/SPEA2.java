 ...
 * objectives.
 * <p>
 * Note: First, there is a naming difference between this implementation and the
 * original SPEA2 paper. The original SPEA2 paper defines a "population" and an
 * "archive", but the population is really the offspring and the archive is the
 * population. Secondly, the SPEA2 paper defines a parameter
 * {@code k = sqrt(population.size())} for computing a crowding-based niching
 * factor. The SPEA2 C implementation in PISA (written by the same authors as
 * the paper) recommends using {@code k=1} for performance reasons. This
...
public class SPEA2 extends AbstractEvolutionaryAlgorithm {
     ...
     * @param k niching parameter specifying that crowding is computed using the
     * {@code k}-th nearest neighbor, recommend {@code k=1}
     */
    ...
    public static class MutableDistanceMap {
         ...
         * distance to its nearest neighbor. Ties are broken by looking at the
         * next nearest neighbor repeatedly until a difference is found.
         *
    ...
    public class StrengthFitnessEvaluator implements FitnessEvaluator {
        ...
        /**
         * Constructs a new fitness evaluator for computing the strength measure
         * with crowding-based niching.
         *
