 ...
 * @author renansantos
 */
public class OnCLMOEAD extends AbstractAlgorithm {
     ...
     */
    private List<OnCLMOEAD.Individual> population;
...
//        hc.printDissimilarity();
    ...
    private void initializePopulation(int populationSize) {
        ...
        if (weightGenerator == null) {
            ...
            for (double[] weight : weights) {
                population.add(new OnCLMOEAD.Individual(weight));
            }
        } else {
    ...
    private void initializeNeighborhoods() {
        List<OnCLMOEAD.Individual> sortedPopulation = new ArrayList<>(
                population);

        for (OnCLMOEAD.Individual individual : population) {
            Collections.sort(sortedPopulation, new OnCLMOEAD.WeightSorter(individual));
    ...
    private List<Integer> getMatingIndices(int index) {
        ...
        if (PRNG.nextDouble() <= delta) {
            for (OnCLMOEAD.Individual individual : population.get(index).getNeighbors()) {
    ...
    private void updateSolution(Solution solution,
        ...
        for (int i = 0; i < matingIndices.size(); i++) {
            OnCLMOEAD.Individual individual = population.get(matingIndices.get(i));
    ...
    private static class MOEADState implements Serializable {
         ...
         */
        private final List<OnCLMOEAD.Individual> population;
         ...
         * {@code MOEAD} instance
         */
        public MOEADState(List<OnCLMOEAD.Individual> population, double[] idealPoint,
         ...
         */
        public List<OnCLMOEAD.Individual> getPopulation() {
            return population;
        }
    ...
    public double[][] getMatrixOfObjetivesFromIndividuals(List<OnCLMOEAD.Individual> population, List<Double> parameters) {
        int rows = population.size();
        int columns = population.get(0).getSolution().getObjectives().length;
        double[][] matrix = new double[rows][columns];
    ...
    public List<Solution> getSolutionListFromPopulationIndividuals(List<OnCLMOEAD.Individual> population) {
        List<Solution> populationList = new ArrayList<>();
        for (OnCLMOEAD.Individual solution : population) {
